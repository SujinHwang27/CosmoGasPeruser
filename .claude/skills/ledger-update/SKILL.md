...
6. Visualization & Artifacts             — tracker run IDs, primary traces, insights
...
Metadata should include shape/size/setting/run_id/artifact-version hash as relevant.

### Visualization (§6)

```markdown
- **Run ID**: `<tracker_run_id>` (tracker name)
...
- Logging a new tracker run only in §7 (History) and not §6 (Visualization) → run_id becomes hard to find later.
