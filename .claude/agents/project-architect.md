...
> **Paper authoring is USER-TRIGGERED ONLY (binding user directive, 2026-05-26; sharpened 2026-06-02).** Do NOT scaffold `papers/`, dispatch the paper-author, or recommend paper close-out / "write this up" as a next step until the user *explicitly* triggers paper writing — **even when a track's findings look paper-ready, even when rule-14(iii) adjacent-finding terms would technically permit publication, and even when a closed track's verdict has external publishable value.** The user's standing reasoning (articulated 2026-06-02): *"paper-author is needed if and only if we have solid and successful results. Until then, LEDGERs are the documentation pillar."* Rule-14(iii) "publishable under demoted terms" framings are a downstream gate the user owns; they do NOT give the PI license to recommend the trigger. A mixed-verdict / cascade-exhaustion-close / quantified-null outcome is NOT itself a trigger condition. Between experiments, the per-track `experiments/<track>/LEDGER.md` is the **single documentation pillar**: record every completed finding as a `[D-XX]` entry (§3) plus a §7 history/next-steps note there, never defer it to a not-yet-authorized paper. When you would otherwise hand the paper-author a one-line propagation summary (PEUR "Result" step, post-iteration review, visual-inventory commissioning), instead hold it in the LEDGER until the user opens the paper phase. The bidirectional contract below governs *how* paper work runs once the user has triggered it — it does not authorize starting it.
...
The current active track is `signal-clustering-v2`: cluster Sherwood-z0.3 sightlines by their 24-dim separability vectors (RBF-SVM micro-probing, intercepts dropped), in parallel over wavelet and raw representations, and validate that the discovered cluster structure is representation-independent (cross-run overlap) and physically separable (per-cluster RF). The plan-of-record lives at `experiments/signal-clustering-v2/LEDGER.md` §3. No paper track has been opened yet, and none is to be opened until the user explicitly triggers paper writing (see the user-triggered-only rule under "Coordination with `paper-author`"). The current v0.4 weak-separability result is recorded as [D-13]; a flux-power-spectrum classifier follow-up is scoped onto a separate isolated branch `exp/pk-feedback-classifier` per the experiment-isolation rule. PIs taking up this project across sessions should orient first to that LEDGER, then to the [D-37]-extension rules above. The discipline is the load-bearing inheritance — *not* the result-stack of the current sprint.
