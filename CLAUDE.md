...
- `.claude/skills/` — `ledger-update` (LEDGER write contract), `mlflow-run` (MLflow run contract + sync), `dvc-track` (DVC pipeline + heavy-artifact contract), `data-export` ([D-31] outbound data-export contract — `src/core/export.py` + `scripts/export/` + `results/exports/<consumer>/MANIFEST.md` for external consumers like selements-website; data-engineer-owned), `skill-transplant` (graft a capability from another agentic repo), `juno-hpc` (UTD Juno HPC submission contract — SSH/SLURM/rsync for jobs that exceed the local laptop budget, e.g. the `exp/pk-feedback-classifier` P_F(k) probe).
