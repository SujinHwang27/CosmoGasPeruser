...
- **Residual provenance caveat:** particle resolution (512³ vs 1024³) is literature-pinned matched (Bolton+2017, "80-512" for classes 1-3) but NOT confirmable from on-disk metadata (`data/raw/` is DVC-pointer-only; the preprocessed README is a 1-line stub). Box size and IC geometry ARE on-disk-confirmed matched. Until `data/raw` (or the Sherwood release manifest) is pulled and the 512³ match is verified, the honest verb ceiling is **"weak P_F(k)-shape signal consistent with the StellarWind-vs-WindAGN feedback difference"**, not "weak feedback signal" unconditionally. Class 4's IC/resolution is formally UNRESOLVED in the cited papers.
...
**Single recommended pre-publication item (cheap, decisive):** pull `data/raw` (or obtain the Sherwood release manifest) and confirm the 512³ particle-resolution match for classes 1-3 (and resolve class-4 provenance). This promotes the H2 verb from "P(k)-shape signal consistent with feedback" to "weak feedback signal" unconditionally. It is a provenance/paperwork task, not compute.
