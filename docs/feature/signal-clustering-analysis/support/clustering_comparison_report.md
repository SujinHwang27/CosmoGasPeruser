...
## 1. Overview
This report analyzes the stability and migration of micro-classifier "personalities" between two different clustering scales ($k=5$ and $k=8$). We identify which separability groups remain robust and which ones dissolve into the background when dimensionality is restricted.
...
| **Static 2D** | `../../../data/feature_discovery/clustering_results_k5/classifier_clusters_umap_k5.png` | `../../../data/feature_discovery/clustering_results/classifier_clusters_umap_k8.png` |
| **Primary Goal** | Simplified separability mapping | **High-resolution Signal Discovery** |
...
*   In $k=8$, **K8_G5** consisted of **329 indices** forming a tight, distinct separability island probably sensitive to extreme physics (like strong AGN feedback).
...
*   **Combinatorial Limit**: With 4 classes, there are 6 distinct binary pairings (OvO). The separability "personality" of an index is a result of how these 6 boundaries are arranged.
...
The micro-classifier separability vectors have **30 dimensions** (24 decision values + 6 intercepts). 
...
The prime objective of this project is to find features that **react** to different physics. If an index's SVM separability vector is in an "Island" (far from the bulk), it means its classification behavior is unique.
