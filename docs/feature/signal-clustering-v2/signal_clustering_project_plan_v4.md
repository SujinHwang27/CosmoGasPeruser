...
2. Find best one-to-one cluster matches via the **Hungarian algorithm** (`scipy.optimize.linear_sum_assignment` on the negated contingency matrix). This finds the unique pairing of wavelet→raw clusters that maximizes total shared sightlines, subject to the constraint that each wavelet cluster is matched to exactly one raw cluster and vice versa. The result is the `best_raw_match` column in `cross_run_overlap_summary.csv`.
...
## Stage 6 — Cluster-Specific Random Forest Classifiers

**What to write:** `src/rf_classifier.py` · `scripts/run_rf.py`

Train a separate 4-class Random Forest classifier for each of the K=5 clusters. The hypothesis: cluster-specific classifiers, trained only on sightlines within that cluster's separability regime, can specialize their decision boundaries and outperform a single global classifier.

**Input per cluster:**
- **Features:** Original spectral representation (wavelet 2048-dim or raw absorption A=1−F, 2048-dim) — the same features used in Stage 2 probing
- **Labels:** 4 physics classes (1–4)
- **Sightline selection:** Only sightlines assigned to that cluster by Stage 3 KMeans labels

**Method:**
1. For each cluster $c$ in [0, 4]: extract sightline indices where `labels == c`
2. For each of the 4 classes, slice the corresponding feature array at those indices → combine into `X_cluster`, `y_cluster`
3. 80/20 stratified train/test split
4. `GridSearchCV` with `StratifiedKFold(n_splits=3)`, scoring `f1_weighted`
5. Parameter grid: `n_estimators=[100]`, `max_depth=[25]`, `min_samples_split=[100, 200]`, `min_samples_leaf=[50, 100]`, `max_features=['sqrt', 0.1]`
6. Log parameters, CV F1, test F1, test accuracy, confusion matrix, and trained model to MLflow (nested runs under a parent run per flavor)

**What to write in `src/rf_classifier.py`:**
- `prep_cluster_data(X_list, cluster_indices) → (X_cluster, y_cluster)`
- `train_rf_for_cluster(X, y, cluster_id, run_flavor) → (best_estimator, best_params, cv_score, test_score, test_acc)`

**What to write in `scripts/run_rf.py`:**
- CLI: `python run_rf.py --run wavelet|raw|both --k 5`
- For each flavor: loads data, loads cluster labels, loops over clusters, trains RF, saves summary CSV

**Outputs:**

| File | Description |
|---|---|
| `results/signal_clustering_v2/rf_summary_wavelet_k5.csv` | Per-cluster: cluster_id, size, CV F1, test F1, test accuracy, best hyperparameters |
| `results/signal_clustering_v2/rf_summary_raw_k5.csv` | Same for raw run |

**Produce:**

| # | Output | Description |
|:---:|---|---|
| T9 | `rf_summary_wavelet_k5.csv` | Per-cluster RF performance summary, wavelet run |
| T10 | `rf_summary_raw_k5.csv` | Per-cluster RF performance summary, raw run |
| P19 | `fig_score_dist_wavelet_k5.png` | 100% stacked bar chart of RF prediction scores per cluster, wavelet |
| P20 | `fig_score_dist_raw_k5.png` | Same for raw run |
| P21 | `fig_drift_animation.mp4` / `.gif` | Animated 2D UMAP: sightlines start at raw UMAP positions (colored by raw cluster), then move to wavelet UMAP positions (drifting sightlines change color to wavelet cluster; stable sightlines keep same color). Uses shared UMAP fit on concatenated fingerprints |
...
| `src/drift_animation.py` | `create_drift_animation()` — animated 2D UMAP showing cluster membership drift between raw→wavelet | ✓ done |
...
| `src/rf_classifier.py` | `prep_cluster_data()` · `train_rf_for_cluster()` — per-cluster RF with GridSearchCV + MLflow | ✓ done |
| `scripts/run_rf.py` | CLI for Stage 6 — `--run wavelet|raw|both --k 5` | ✓ done |
| `scripts/run_all.py` | End-to-end orchestrator — calls all scripts in order | ← write |
...
| T2 | `results/signal_clustering_v2/stage2_separability_vector_summary.csv` | `run_probe.py` |
...
| T7 | `results/signal_clustering_v2/variance_audit.csv` | `run_audit.py` |
| T8 | `results/signal_clustering_v2/rf_summary_wavelet_k5.csv` | `run_rf.py` |
| T9 | `results/signal_clustering_v2/rf_summary_raw_k5.csv` | `run_rf.py` |
...
| P19 | `figs/signal_clustering_v2/fig_score_dist_wavelet_k5.png` | — | `run_rf.py` |
| P20 | `figs/signal_clustering_v2/fig_score_dist_raw_k5.png` | — | `run_rf.py` |
...
│   └── rf_classifier.py                         ✓ done: per-cluster RF with MLflow
│
├── scripts/
...
│       ├── variance_audit.csv                   ← T7
│       ├── rf_summary_wavelet_k5.csv            ← T8
│       └── rf_summary_raw_k5.csv                ← T9
│
└── figs/
...
| 6 | ✓ | Cluster-specific RF classifiers with MLflow logging |
